    ...
    for (int x = minX; x <= maxX; ++x) {
      ...
      if (currentPixel.draw) {
        ...
        if (TEXTURE and LIGHTNING) {
          ...
          glm::vec3 bary = extractBarycentric(modelIdx, triIdx, x, y);
          framebuffer[y][x][0] = static_cast<float>(color.r)*bary.x;  //R
          framebuffer[y][x][1] = static_cast<float>(color.g)*bary.y;  //G
          framebuffer[y][x][2] = static_cast<float>(color.b)*bary.z;  //B
        }
